...
export const HeroSection: React.FC<HeroSectionProps> = ({ config, onOpenCustomizer, onOpenAuth }) => {
  ...
  useEffect(() => {
    const calculateCountdown = () => {
      const now = new Date();
      let birthMonth = 7; // Default August (0-indexed 7)
      let birthDay = 15;  // Default 15th

      if (config.birthdayDate) {
        const parts = config.birthdayDate.split('-');
        if (parts.length === 3) {
          ...
          const parsed = new Date(config.birthdayDate);
      ...
      // Create target date for current year at midnight in local time
      let target = new Date(now.getFullYear(), birthMonth, birthDay, 0, 0, 0);

      // If today is after the target date for this year, check if today is the birthday or if it's already passed
      if (now.getTime() > target.getTime()) {
        ...
        if (!isToday) {
          ...
          target = new Date(now.getFullYear() + 1, birthMonth, birthDay, 0, 0, 0);
        }
      }
